...
この記事は [Ethereum Advent Calendar](https://qiita.com/advent-calendar/2017/ethereum) の 11 日目の記事です。表題の通り、この記事では、今年行われた [Raiden](https://raiden.network) の ICO と向き合います。敢えて Raiden そのものとも向き合いません。ご承知ください。

<!--more-->

![raiden_1](/my-images/entry/raiden_1.png)
...
まず、大前提として、自分は ICO を肯定的に捉えています。しかし、今の ICO （特にクラウドセール）が最適なカタチをしているとは思っていません。端的に言うと、
...
- もっとクラウドセールの民主性を高めるべきではないか
  - かつ、コミュニティの価値向上をプロジェクトの価値向上の主軸に据え、クラウドセール期間から、支援者を良い意味で共犯者として巻き込んだコミュニティを形成すべきではないか
- せっかくスマートコントラクトがあるのだから、コードによるガバナンスをもっとうまく活用すべきではないか
...
とのことです。「早く入札しないと、先に誰かに落札されてしまう」といった心理状態を利用して入札を誘発し、それによって取引の高速化を図る、というのがキモのようです。
...
実際に使われたコントラクトは [こちら](https://etherscan.io/address/0xb5e5585d0057501c91c48094029a6f4fb10b5a01#code) です。実物を確認したい方はご参照ください。
...
![raiden_2](/my-images/entry/raiden_2.png)
...
なんと、総トークン販売量の約 95 % が売れ残り、それが全て Gnosis チームに引き渡されたということになります。Gnosis のダッチオークションは開始 10 分で調達額のキャップに達してしまったことで、このような残念な結果となってしまったのです。
...
- 一定期間、プロジェクト側が決めた販売価格で ERC20 トークンを販売する形式
  - 販売価格は、販売期間を通じて一定、もしくは一定期間ごとに上昇する

販売価格が一定期間ごとに上昇するパターンは、「1 週目は 1 トークンあたり 0.01 ETH、2 週目は 1 トークンあたり 0.02 ETH、3 週目は、、」といったイメージです。
...
実際、執筆時点における [CoinMarketCap](https://coinmarketcap.com/currencies/raiden-network-token) のチャートを見る限り、RDN の価格変動は比較的落ち着いているように思えます。

![raiden_3](/my-images/entry/raiden_3.png)
...
個人的には、よりシンプルかつ民主性のあるスキームが必要なのではないかと考えています。

<br />
### 4-3. 重要な要素の不確定性

重要な要素とは、トークンの販売価格のことです。これは参加者にとって「気になる」値でしょう。ダッチオークションが終了するまでこれが不確定であるということは、
